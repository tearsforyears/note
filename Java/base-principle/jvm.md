...
> 逃逸分析是编译优化中的重要技术,是一种确定**指针动态范围**的方法,分析在程序的哪些地方可以访问到指针。它涉及到**指针分析**和**形状分析**
>

这是一种可以有效减少Java 程序中同步负载和内存堆分配压力的跨函数全局数据流分析算法.通过逃逸分析,Hotspot编译器能够分析出一个新的对象的引用的使用范围从而决定是否要将这个对象分配到堆上.其逃逸分析的主要依据是
...
- 将堆分配转化为栈分配(TLAB)。如果某个对象在子程序中被分配，并且指向该对象的指针永远不会逃逸，该对象就可以在分配在栈上，而不是在堆上。在有垃圾收集的语言中，这种优化可以降低垃圾收集器运行的频率。
- 同步消除。如果发现某个对象只能从一个线程可访问，那么在这个对象上的操作可以不需要同步。
- 分离对象或标量替换。如果某个对象的访问方式不要求该对象是一个连续的内存结构，那么对象的部分（或全部）可以不存储在内存，而是存储在CPU寄存器中。减少对象头，提高回收效率
...
所谓的方法内联,即是编译时期开展的函数,是一种直接使用函数代码段替换某一方法段的技术.JVM 对 java 的方法进行了优化,不过我们一般看不到内联函数,jvm 能做到热点分析.在 c++ 中,可以通过`inline`关键字指定方法内联.方法内联会减少额外的栈开销,但会增加编译后的体积,需要慎用.而 jvm 则不需要程序员去显示的使用如上代码

jvm 的内联条件有如下

1. 热点代码,服务端的编译模式是 10000 次, 客户端则是 1500 次 由 `-XX:CompileThreshold` 来调整
2. 方法大小有限制,被编译的机器码放在 code cache 中,如果方法体太大,则能缓存的热点方法就少
3. `private static final` 修饰的代码,可以增加被内联的机会, jvm 可以直接内联上述关键字,而 public protected 修饰的方法则需要jvm 进行类型的判断.因为其他关键字有可能被子类继承不能直接确定内联的代码.

- 在 JDK 中 `@DontInline` 标注的方法不会被内联
- native 方法无法被内联

![](https://upload-images.jianshu.io/upload_images/17426433-c0b1a7852f26814f.png?imageMogr2/auto-orient/strip|imageView2/2/w/1200/format/webp)
...
func fact(n int) int {
    if n <= 0 {
        ...
        return n * fact(n - 1) // 会压栈 n 保存操作数,n-1传递至一下层, fact 也会压栈层层保存
    }
}

func fact(n int, r int) int {
    if n <= 0 {
        return 1 * r
    } else {
        return fact(n - 1, r * n) // 不会压栈操作数,传递n-1,r*n至一下层
      	// 因为可以立即返回得到值,所以无需真正压栈,这就是尾递归优化
    }
}
```

上面是一种写法上的优化,在实际压栈的过程不优化上述两个都一样,对于所有尾递归函数可以如下优化

```go
func _fact(_n int, _r int) int { // <= 迭代函数非常简单,就是更新迭代变量而已
  var n = _n
  var r = _r
}
func fact(_n int , _r int) int { // <= _n, _r 用作初始化变量
    var n = _n
