...
下面三种分配器可以在一些组件上上使用 mysql redis ngingx等.但需要重新编译 `make && make install`,redis支持下面三种内存分配器的使用
...
- **allkeys-random**：**随机**驱逐密钥以便为添加的新数据腾出空间。
- **volatile-random**：**随机**驱逐密钥以便为添加的新数据腾出空间，但仅驱逐具有**过期设置的**密钥。
